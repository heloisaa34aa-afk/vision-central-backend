...
create table if not exists public.feed_source_media (
  source_id text primary key references public.feed_sources(id) on delete cascade,
  midia_id text not null references public.midias(id) on delete cascade,
...
create or replace function public.increment_tv_config_revision(p_tv_id text)
returns void
language sql
security definer
...
grant execute on function public.increment_tv_config_revision(text) to anon, authenticated, service_role;
